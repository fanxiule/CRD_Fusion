...
class KITTI2015TestDataset(CRDFusionDataset):
    def __init__(self, data_path, max_disp, resized_h, resized_w, conf_thres, imgnet_norm=True, sanity=False):
        """
        Dataset to load and prepare data from KITTI 2015 test split
        ...
        """
        super(KITTI2015TestDataset, self).__init__(data_path, max_disp, 1, resized_h, resized_w, conf_thres, False,
                                                   imgnet_norm, sanity)
        self.data_path = os.path.join(self.data_path, "testing")
        img_list_path = os.path.join(self.data_path, "image_2")
